...
import com.zluolan.zojbackendmodel.dto.question.*;
...
public class QuestionController {
     ...
     * 从请求头获取用户ID（允许未登录）
     */
    private Long getUserIdFromHeaderPermitNull(HttpServletRequest request) {
        String userIdStr = request.getHeader("X-User-Id");
        if (userIdStr == null) {
            return null;
        }
    ...
    public BaseResponse<Long> addQuestion(@RequestBody QuestionAddRequest questionAddRequest, HttpServletRequest request) {
        log.debug("开始创建题目，请求参数: {}", questionAddRequest);
        ...
        log.debug("准备保存题目，题目信息: {}", question);
        
        boolean result = questionService.save(question);
        ThrowUtils.throwIf(!result, ErrorCode.OPERATION_ERROR);
        long newQuestionId = question.getId();
        
        log.debug("题目创建成功，题目ID: {}", newQuestionId);
